...
def RunIR():
    ...
    network = cv2.dnn.readNetFromDarknet('cardDetection/train.cfg',
                                        'cardDetection/card_chip.weights')
    ...
    def pairs(results):
        ...
        return({str(sorted((i,j))):(results[i][0],results[j][0]) for i,j in enumerate(cards) if cards[j] == i}).values()
    ...
    while True:
        ...
        if len(results) > 0:
            ...
            for i in results.flatten():
                ...
                print("Location of",labels[int(class_numbers[i])],"=", x,y, "Confidence =",text_box_current[3:])
